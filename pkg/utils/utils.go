...
func CollectPods(namespace string, matchLabels map[string]string, logger log2.FieldLogger, client *kubernetes.Clientset,
...
func retry(ctx context.Context, times int, period time.Duration, fn func() error) error {
	...
	for i := 1; i < times && err != nil; i += 1 {
		log2.Infof("attempt %v, result: %v, retry in %v", i+1, trace.DebugReport(err), period)
		select {
		case <-ctx.Done():
			log2.Infof("context is closing, return")
			return err
...
func withRecover(fn func() error, recoverFn func() error) error {
	...
	defer func() {
		if !shouldRecover {
			log2.Infof("no recovery needed, returning")
			return
		}
		log2.Infof("need to recover")
		err := recoverFn()
		if err != nil {
			log2.Error(trace.DebugReport(err))
			return
		}
		log2.Infof("recovered successfully")
		return
	}()
...
func CheckRunning(pods map[string]v1.Pod, nodes []v1.Node, logger log2.FieldLogger) error {
...
func checkRunningAndReady(pods map[string]v1.Pod, nodes []v1.Node, logger log2.FieldLogger) (bool, error) {
...
func deletePodsList(podIface corev1.PodInterface, pods []v1.Pod, logger log2.FieldLogger) error {
...
func DeletePods(podIface corev1.PodInterface, pods map[string]v1.Pod, logger log2.FieldLogger) error {
	for _, pod := range pods {
		logger.Debugf("deleting pod %v", pod.Name)
		err := ConvertError(podIface.Delete(pod.Name, nil))
		if err != nil && !trace.IsNotFound(err) {
